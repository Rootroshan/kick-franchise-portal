...
export default async function AuditLogPage({ searchParams }: { searchParams: Record<string, string | string[] | undefined> }) {
  ...
  const columns: Parameters<typeof AuditLogListSection>[0]["columns"] = [
    {
      key: "action",
      header: "Action",
      cell: (l: AuditRow) => (
    ...
    { key: "actor", header: "Actor", hideOnMobile: true, cell: (l: AuditRow) => <span className="font-mono text-xs text-muted-foreground">{l.actorId}</span> },
    { key: "role", header: "Role", cell: (l: AuditRow) => <StatusBadge status={l.role} /> },
    { key: "brand", header: "Brand", hideOnMobile: true, cell: (l: AuditRow) => <span className="text-muted-foreground">{l.brandName ?? "Platform"}</span> },
    { key: "when", header: "When", cell: (l: AuditRow) => <span className="text-muted-foreground">{l.createdAt.toLocaleString()}</span> },
  ];
