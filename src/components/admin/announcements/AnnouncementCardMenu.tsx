...
import { MoreVertical, Send, Archive, Pin, PinOff, FileBarChart } from "lucide-react";
import { toast } from "sonner";
import { cn } from "@/lib/utils";
import { LoadingOverlay } from "@/components/ui/LoadingOverlay";
import { setAnnouncementStatusAction, toggleAnnouncementPinAction } from "@/app/admin/announcements/announcementActions";

type Kind = "publish" | "expire" | "pin" | "report";
...
export function AnnouncementCardMenu({
  ...
  const items: Array<{ icon: typeof Send; label: string; kind: Kind }> = [
  ...
  const onPick = (kind: Kind) => {
    ...
    startTransition(async () => {
      try {
        const result =
          kind === "publish"
            ? await setAnnouncementStatusAction(id, "PUBLISHED")
            : kind === "expire"
            ? await setAnnouncementStatusAction(id, "EXPIRED")
            : await toggleAnnouncementPinAction(id, !isPinned);
        if (result.ok) {
